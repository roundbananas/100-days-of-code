...
class Flasher
{
  ...
  int ledState;   //used to set the LED state
  ...
  void UpdateLed()
  {
    ...
    if((ledState == HIGH) && (currentMillis - previousMillis >= OnDuration))
    {
      ledState = LOW; //change the state to off
      previousMillis = currentMillis; // remember the time
      leds[ledNumber] = CRGB::Black;     // change colour of actual led to black (off)
      FastLED.show(); //update the actual LED
    }
    else if ((ledState == LOW) && (currentMillis - previousMillis >= OffDuration))
    {
      ledState = HIGH; //change state to on
