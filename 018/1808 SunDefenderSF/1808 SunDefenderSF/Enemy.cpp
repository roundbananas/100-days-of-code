...
void Enemy::updateD(double t, VertexArray& mountainsVA)
{
    ...
    if (!m_Shooting && (m_VPosition == mountainsVA[4].position || m_VPosition == mountainsVA[6].position || m_VPosition == mountainsVA[8].position || m_VPosition == mountainsVA[10].position || m_VPosition == mountainsVA[11].position))
    {
        srand((int)time(0) * t);   //seed random number generator with current position
    ...
    //if we've been shooting for 3 seconds, then stop shooting
    if(m_Shooting && (t - m_lastUpdate) >= 3)
    {
        m_Shooting = false;
    }
    
    //if we're due to move, work out where to move
    //if we're shooting, don't move
    if (t - m_lastUpdate >= m_LingerTime && !m_Shooting)
    {
