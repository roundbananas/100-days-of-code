...
void Engine::m_Update(double currentTime, double lastTime)
{
    ...
    if (mode == GameMode::PLAYING)
    {
        ...
        //Update MtnProjectiles. The mountain projectile is turned on when the canon's projectile reaches the base of the mountain.
        ...
        for(int i = 0; i < m_NumEnemies; i++)
        {
            for (int j = 0; j < m_numCanons; j++)
            {
                if (mtnProjectiles[j].getOn() && (enemies[i].getPosition() == ( options )))
                {
                    enemies[i].hit();
