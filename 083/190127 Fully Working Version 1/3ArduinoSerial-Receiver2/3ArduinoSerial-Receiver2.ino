...
SevSeg display1; //Current round
SevSeg display2; //Best rounds
  ...
  byte display1_digitPins[1] = {8};
  byte display1_segmentPins[8] = {13, 12, 11, 10, 9, 4, 5, 6}; //DP used to indicate round 10+
  
  byte display2_digitPins[1] = {7};
  byte display2_segmentPins[8] = {A5, A4, A3, A2, A1, A0, 2, 3}; //
...
void setup() {
  ...
  for (int i = 0; i < 8, i++) {pinMode(display2_segmentPins[i], OUTPUT);}
  ...
  //Set displays to 0
  display1.setNumber(currentValue, -1); //'-1' toggles DP off
...
void handlePreviousState()
{
  ...
  {
   case DISPLAY1:
    if (!MaxedOut_Display1)  display1.setNumber(currentValue, -1);
    else 
    {
    ...
    break;
  case DISPLAY2:
    if (!MaxedOut_Display2)  display2.setNumber(currentValue, -1);
    else 
    {
      display2.setNumber((currentValue), 0);
    }
    break;
  } //end switch
