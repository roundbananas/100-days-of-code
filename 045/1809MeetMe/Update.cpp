...
void Engine::m_Update(unsigned long dt, unsigned long t)
{
  ...
  {       
   ...
   { 
      ...
      {
        if (m_Players[j].getHasFired() && i == m_Players[j].getBulletIndex())
        {
          ...
          if (m_Bullets[i].BIsInFlight()) 
            {
              ...
              if (m_Bullets[i].getHeadBPos() <= m_Bullets[j].getHeadAPos())
              {
                 ...
                 */
                  ...
                  {
                    if (i == m_Players[k].getBulletIndex())
                    {
                      m_Players[k].setIsExploding();
                      break;
                    }
                  }
                  ...
                  explodeOnStrip = getExplodeStrip(j); //this function checks which strip (player) the j bullet is associated with
