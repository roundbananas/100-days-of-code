...
void Bullet::spawn(int pixels, int dataPin, int clockPin, int head, int tail, int colour[])
{
  ...
  m_RedValues[0] = colour[0];
  m_GreenValues[0] = colour[1];
  m_BlueValues[0] = colour[2];
  ...
  for (int i = (m_Head - m_Tail) - 1; i > 1; i--)
  {
    ...
    m_RedValues[i] = factor * colour[0];
    m_GreenValues[i] = factor * colour[1];
    m_BlueValues[i] = factor * colour[2];
  }
}
