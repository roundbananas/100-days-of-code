...
/*
  ...
  {       
   ...
   for (int i = 0; i < m_NumExplosions; i++)
   {
    ...
    {
      if(m_Level < m_BestRounds)
      {
        m_BestRounds = m_Level;
        m_NewBestRounds = true;
      }
       m_mode = Modes::GAME_OVER;
    }
    else if ((m_CurrentExplosion == m_NumPlayers - 1) && (t - m_LastExplosionStartedAt > 500)) //wait 500ms before changing modes, to help let people make transition
    {
       loadLevel();
