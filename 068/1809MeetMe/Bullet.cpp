...
void Bullet::updateBullet(int num_Pixels, float bulletSpeed, unsigned long t, bool boostButtonRead)
{
  //what's the instantaneous speed of the bullet?
  if(!m_Boosting && boostButtonRead)  //just started boosting
  {
    m_BoostStartedAt = t;
    m_Boosting = true;
  ...
  else if (m_Boosting)  //still boosting
  {
    m_Speed = bulletSpeed * m_BulletSpeedFactor * m_BoostFactor;
