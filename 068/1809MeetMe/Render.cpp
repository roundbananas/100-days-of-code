...
{  
  ...
  { 
    if(!m_IdleFlag)
    {
      ...
      //turn on Player button lights
      for (int i = 0; i < 3; i++)
      {
        m_PlayerButtonLeds[i].TurnOn();
      }
  ...
  {  
      ...
      if (!m_CountdownFlag)
      {
        ...
        UpdateStrips(t);
        //turn off New Game light
        m_NewGameButtonLED.TurnOff();
...
if((m_mode == Modes::GAME_OVER))
  ...
  if(m_mode == Modes::PLAYING)
  {
    for (int i = 0; i < m_NumPlayers; i++)
    { 
      int mode = m_PlayerButtonLeds[i].GetMode();
      switch (mode)
      {
        case 0: //off
            m_PlayerButtonLeds[i].TurnOff();
