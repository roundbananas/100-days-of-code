...
class Bullet
{
  private:
    ...
    float m_BulletSpeedFactor = 0.045f; //fudge factor to allow enough sensivity on pot interface, while making the bullet fly at a reasonable speed.
                              //was 0.02f, which was not so exciting at max speed (without boost) for 130led strip. Provide a turbo option to boost the speed instead.

    //Manage boosting
    bool m_Boosting = false;
    int m_BoostsRemaining = 1; 
    int m_BoostFactor = 2.5;  //speed x2
    unsigned long m_BoostDuration = 750; //ms
    unsigned long m_BoostStartedAt;
