...
/*
  ...
  {           
   //Update the bullet positions and player button lights
      for (int i = 0; i < m_NumPlayers; i++)
      {
        if (!m_Players[i].isExploding() && m_Players[i].getHasFired())
        {
          ...
          if (m_Bullets[i].getIsBoosting()) //flash boost button while it's boosting.
          {
            m_PlayerBoostLeds[i].SetMode(2);
          }
          else if (int var = m_Bullets[i].getBoostsRemaining() <= 0)
          {
            m_PlayerBoostLeds[i].SetMode(0); //turn off boost led if they've boosted already
          }
          else 
          {
            m_PlayerBoostLeds[i].SetMode(1); //turn on boost button led
          }
          
          m_Bullets[i].updateBullet(m_stripLengthArray[i], m_SlidePots[i].updateSlidePot(), t); 
        }
        else if (!m_Players[i].isExploding() && !m_Players[i].getHasFired()) //If player hasn't fired, we can't refer to the bullets because they haven't been associated with the player yet.
        {
            ...
            m_PlayerBoostLeds[i].SetMode(0); //indicate that they can't push the boost button yet
        }    
        else if (m_Players[i].isExploding()) //turn off both their button leds to indicate they can't press either anymore.
        {
 ...
 /*
  ...
  { 
    ...
    {       
        ...
        //3 seconds countdown. See notes in Engine.h before declaration of m_CountDownDuration variable.
        //+1 added to shift the countdown pixels up one pixel along the strip, 
        //because the first pixel is actually hidden in the physical structure.
        //so we stop the countdown once it gets to 1, not 0.
 ...
 /*
  ...
  { 
    ...
    //set New Game button LED to flashing
