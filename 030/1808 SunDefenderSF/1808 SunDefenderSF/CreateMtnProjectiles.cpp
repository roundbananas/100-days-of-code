...
MtnProjectiles* createMtnProjectiles(int numCanons, VertexArray& mountainsVA, VertexArray& projectileTargetsVA, VertexArray& mtnProjectilePosVA)
{
    ...
    for(int i = 0; i < 5; i++)
    {
        ...
        //Associate mountain peak to mtnProjectile (for collision detection with enemy)
        Vector2f peakPosition;
        switch(i)
        {
            case 0: //mtnProjectile #1 associates with mtn peak #3
                peakPosition.x = mountainsVA[3].position.x;
                peakPosition.y = mountainsVA[3].position.y;
                break;
            case 1:
                peakPosition.x = mountainsVA[5].position.x;
                peakPosition.y = mountainsVA[5].position.y;
                break;
            case 2:
                peakPosition.x = mountainsVA[7].position.x;
                peakPosition.y = mountainsVA[7].position.y;
                break;
            case 3:
                peakPosition.x = mountainsVA[9].position.x;
                peakPosition.y = mountainsVA[9].position.y;
                break;
            case 4:
                peakPosition.x = mountainsVA[10].position.x;
                peakPosition.y = mountainsVA[10].position.y;
                break;
        }
        
        mtnProjectiles[i].spawn(spritePosition, i, triggerPosition, peakPosition);
    }
