...
EnemyLaser* createEnemyLasers(VertexArray& laserVA, VertexArray& mountainsVA)
{
    ...
    for (int i = 0; i < 5 ; i++)
    {
        ...
        {
            case 0:
                peakPos.x = mountainsVA[3].position.x;
                peakPos.y = mountainsVA[3].position.y;
                break;
            case 1:
                peakPos.x = mountainsVA[5].position.x;
                peakPos.y = mountainsVA[5].position.y;
                break;
            case 2:
                peakPos.x = mountainsVA[7].position.x;
                peakPos.y = mountainsVA[7].position.y;
                break;
            case 3:
                peakPos.x = mountainsVA[9].position.x;
                peakPos.y = mountainsVA[9].position.y;
                break;
            case 4:
                peakPos.x = mountainsVA[10].position.x;
                peakPos.y = mountainsVA[10].position.y;
                break;
        }
        
        enemyLasers[i].spawn(vPos, peakPos, i);
    }
    return enemyLasers;
}
