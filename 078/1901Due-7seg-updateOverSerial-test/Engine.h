...
class Engine
{
  private:
    ...
    byte m_CurrentRound_7SegPins[8] = {16, 17, 18, 31, 20, 21, 22, 14}; //NOTE pin 14 is not connected. We don't need DP.
   ...
   */
   ...
   // It's a bit of overkill in this case to put it in an array then assign it to the display, but good practice/structure if we have more complex situation.
    int m_DataReceived[m_NumDisplays] = {}; //initialise to 0. Each display only shows one number, so it's a 3 x 1 array.
