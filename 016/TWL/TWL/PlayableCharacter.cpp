...
{
    m_Position.x = position + m_Sprite.getGlobalBounds().width; //shouldn't this just be = position? as it is the character will be shifted to 1 character's width back to the right
    m_Sprite.setPosition(m_Position);
}

void PlayableCharacter::stopJump(float position) //argument missing from video 12:23
{
    //stop a jump early
    m_IsJumping = false;
    m_IsFalling = true;
