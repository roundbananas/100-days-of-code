...
void Engine::m_Input(unsigned long t)
{
  if(m_mode == Modes::PLAYING)
  {
    ...
    {
     Serial.print ("Input: Bullet headApos "); Serial.println (m_Bullets[i].getHeadAPos());
     Serial.print ("Input: Bullet headBpos "); Serial.println (m_Bullets[i].getHeadBPos());
    }

    for (int i=0; i < m_NumPlayers; i++)
    {               
   //   Serial.print ("Input: Player Fired? "); Serial.println (m_Players[i].getHasFired());
      if (m_Buttons[i].updateButton() && !m_Players[i].getHasFired())
      {
          m_Players[i].setHasFired();
          m_Players[i].setBulletIndex(m_LoadedBullet);
          m_Bullets[m_LoadedBullet].shoot(m_SlidePots[i].updateSlidePot(),m_Players[i].getColour()); 
          m_LoadedBullet++;
      }
    }
  }
