...
//#endif
    ...
    for (int i = 0; i < m_NumPlayers; i++)
    {
     ...
     for (int i = 0; i < 2; i++) 
    {
      m_LevelDisplay[i].Spawn(m_SegmentPins[i]); //we're passing the array of pins to the spawn function
    }
...
//Identify what single-digit-number to display on each 7segment display
int Engine::firstDigit(int level)
{
  while (level >= 10)
    level /= 10;
  return level;
...
int Engine::secondDigit (int level)
{
  return (level % 10);
} 
