...
void Engine::m_Update(unsigned long dt, unsigned long t)
{
 ...
 //   m_FramesSinceLastHUDUpdate++;
  ...
  //  {
           ...
           {
            //   Serial.print ("Update: Bullet headApos "); Serial.println (m_Bullets[i].getHeadAPos());
            // Serial.print ("Update: Bullet headBpos "); Serial.println (m_Bullets[i].getHeadBPos());
              for (int j = 0; j < m_NumPlayers; j++)
              {
                if (i == m_Players[j].getBulletIndex() && m_Players[j].getHasFired()) //m_Bullets[i].AIsInFlight()
                {
                  m_Bullets[i].updateBullet(m_stripLengthArray[0], dt); //NEED TO FIX STRIP LENGTH ARRAY INDEX IF STRIPS ARE OF DIFFERENT LENGTHS
                     //  for (int f = 0; f< m_NumBullets; f++)
                    //  {
                   //    Serial.print ("Update2: Bullet headApos "); Serial.println (m_Bullets[f].getHeadAPos());
                   //    Serial.print ("Update2: Bullet headBpos "); Serial.println (m_Bullets[f].getHeadBPos());
                   //   }
