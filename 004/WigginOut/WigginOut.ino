...
State WigginMode(State current_state, unsigned long current_time, bool button_reading, long time_out){
    ...
    //    ResetTimerInterrupt();//stop the timer to allow time for g_stepsToHome to be calculated and the variable updated
...
void Render(State current_state){
   ...
   if(state.modeFunction == IdleMode){
       ...
       flasher3.GoBlack();
     //  if(TCCR1B == bit (CS10)){
     //    Serial.println("Render: turning timer OFF");
     //    ResetTimerInterrupt();
     //  }
   }
   ...
   if(state.modeFunction == WigginMode){
       ...
       flasher3.UpdateLed();
    //   if(TCCR1B != bit (CS10)){
     //     Serial.println("Render: turning timer on");
     //     StartTimerInterrupt();
     //  }
   ...
   if(state.modeFunction == ReturnMode){
       ...
       flasher2.GoBlack();
       flasher3.GoBlack();
       //the timer interupt takes care of returning the stepper to home position.
