...
void Engine::m_Update(double currentTime, double lastTime)
{
    ...
    if (mode == GameMode::PLAYING)
    {
        ...
        for(int i = 0; i < m_NumEnemies; i++)
        {
            ...
            //Play move sound only if they've moved
            Vector2f lastPos = enemies[i].getPosition();
            enemies[i].updateD(currentTime, m_mtnArrayDigital);
            Vector2f newPos = enemies[i].getPosition();
            if(!(lastPos == newPos))
            {
                m_SM.playEnemyMove();
            }
            ...
            for (int j = 0; j< m_NumEnemies; j++)
            {
                if (enemies[j].getJustStoppedShooting() && sun.isAlive())
                {
                    ...
                    if(!sun.isAlive())
                    {
                        ...
                        m_SM.stopBGPlayingMusic();
                        m_SM.playBGMenuMusic();
                        m_SM.stopEnemyLaser();
                        m_SM.playSunTotalDeath();
                    }
                    //play sound each time we lose a segment
                    //work out how to do this sometime!?
                }
            }
        } //End sun being shot
